	...
	public function fetchUserInformation(Request $request){
		...
		if (!$validator->passes()) {
			...
			$userNameRes = DB::table("users")
				...
				$custom = collect(['blocked'=>$blockText,'totalRecords'=>$totalVideos,'large_pic' => $file_path ,'small_pic' => $small_file_path,'name' => $name,'totalVideosLike'=>$totalVideosLike, 'totalFollowings' => $totalFollowing, 'totalFollowers' => $totalFollowers, 'followText' => $followText,'totalVideos'=>Functions::digitsFormate($totalVideos),'isVerified'=>$verified_status,'username'=>$userNameRes->username]);
	...
	public function fetchLoginUserInformation(Request $request){
		...
		if (!$validator->passes()) {
			...
			if($token_res>0){
				...
				$custom = collect(['totalRecords'=>$totalVideos,'large_pic' => $file_path ,'small_pic' => $small_file_path,'name' => $name,'totalVideosLike'=>$totalVideosLike, 'totalFollowings' => $totalFollowing, 'totalFollowers' => $totalFollowers,'totalVideos'=>Functions::digitsFormate($totalVideos),'isVerified'=>$verified_status,'username'=>$userNameRes->username]);
                $userVideos = $custom->merge($userVideos);
