...
class SoundController extends Controller
{
    ...
    public function index(Request $request){
        ...
        if( count($categories) > 0 ) {
			...
			foreach($categories as $key => $value) {
				...
				$sounds = DB::table("sounds as s")->select(DB::raw("s.sound_id,s.duration,title,s.album,case when s.user_id = 0  then concat('".$storagePath."/',sound_name) else concat('".$storagePath."/',s.user_id,'/',sound_name) end as sound_url,duration,s.user_id,tags,ifnull( case when INSTR(u.user_dp,'https://') > 0 THEN u.user_dp ELSE concat('".$userDpPath."/',fs.user_id,'/small/',u.user_dp) END,'".asset('default/music-icon.png')."') as image_url,ifnull(fs.sound_id,0) as fav,s.cat_id,s.used_times,ifnull(s.created_at,'NA') as created_at"))
    ...
    public function getCategorySounds(Request $request){
        ...
        $sounds = DB::table("categories as c")->select(DB::raw("s.sound_id,case when s.album = '' then 'Unknown' else s.album end as album,s.duration,title,case when s.user_id = 0  then concat('".$storagePath."/',sound_name) else concat('".$storagePath."/',s.user_id,'/',sound_name) end as sound_url,duration,s.user_id,tags,ifnull( case when INSTR(u.user_dp,'https://') > 0 THEN u.user_dp ELSE concat('".$userDpPath."/',fs.user_id,'/small/',u.user_dp) END,'".asset('default/music-icon.png')."') as image_url,ifnull(c.cat_name,'No category') as category,ifnull(fs.sound_id,0) as fav,s.cat_id,s.used_times,ifnull(s.created_at,'NA') as created_at"))
    ...
    public function favSounds(Request $request){
        ...
        $sounds = DB::table("favourite_sound as fs")->select(DB::raw("fs.sound_id,title,album,case when fs.user_id = 0  then concat('".$storagePath."/',sound_name) else concat('".$storagePath."/',fs.user_id,'/',sound_name) end as sound_url,duration,fs.user_id,tags,ifnull( case when INSTR(u.user_dp,'https://') > 0 THEN u.user_dp ELSE concat('".$userDpPath."/',fs.user_id,'/small/',u.user_dp) END,'".asset('default/music-icon.png')."') as image_url,ifnull(fs.sound_id,0) as fav,s.used_times,ifnull(s.created_at,'NA') as created_at"))
