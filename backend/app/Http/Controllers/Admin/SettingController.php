...
class SettingController extends Controller
{
    ...
    private function _form_validation($request){
        ...
        if($request->hasFile('site_logo')) {
            ...
            if(isset($request->id) && $request->id>0 && $request->old_site_logo!=''){
                // dd($request->old_img);
       ...
       if($request->hasFile('watermark')) {
            ...
            $functions->createThumbWidth(asset(Storage::url($filenametostore)),100,$path,"small_".$watermark);

            
            
            // $file_path = asset(config('app.profile_path').$request->user_id."/".$fileName);
        }else{
            if(isset($request->id) && $request->id>0 && $request->old_watermark!=''){
    ...
    public function  nsfwSettingUpdate(Request $request)
    {
        ...
        $nsfwData['wad'] = is_null($request->android_video_app_id) ? 0 : $request->android_video_app_id;
    ...
    {   
        ...
        if($request->mail_type=='SG'){
            $env_values = array("SENDGRID_API_KEY"=>$request->api_key, "MAIL_DRIVER"=>"sendgrid","MAIL_FROM_ADDRESS"=>$request->from_email );
        }else{
            $env_values = array("MAIL_HOST"=>$request->mail_host, "MAIL_DRIVER"=>"smtp","MAIL_PORT"=> $request->mail_port ,"MAIL_USERNAME"=>$request->mail_username,"MAIL_PASSWORD"=>$request->mail_password,"MAIL_ENCRYPTION"=>$request->mail_encryption,"MAIL_DRIVER"=>"smtp","MAIL_FROM_ADDRESS"=>$request->from_email );
        }
    ...
    public function  homeSettingUpdate(Request $request)
    {
        ...
        $data['comments_per_page'] = is_null($request->comments_per_page) ? '8' : $request->comments_per_page;
        $data['videos_per_page'] = is_null($request->videos_per_page) ? '12' : $request->videos_per_page;
    ...
    {
        $socialData = array();
        $socialData['google_client_id'] = is_null($request->google_client_id) ? '' : $request->google_client_id;
        $socialData['google_secret'] = is_null($request->google_secret) ? '' : $request->google_secret;
        $socialData['facebook_client_id'] = is_null($request->facebook_client_id) ? '' : $request->facebook_client_id;
        $socialData['facebook_secret'] = is_null($request->facebook_secret) ? '' : $request->facebook_secret;
        $socialData['google_active'] = is_null($request->google_active) ? 0 : $request->google_active;
        $socialData['fb_active'] = is_null($request->fb_active) ? 0 : $request->fb_active;
    //    dd($socialData);
    ...
    {
        $pusherData = array();
        $pusherData['pusher_app_id'] = is_null($request->pusher_app_id) ? '' : $request->pusher_app_id;
        $pusherData['pusher_app_key'] = is_null($request->pusher_app_key) ? '' : $request->pusher_app_key;
        $pusherData['pusher_app_secret'] = is_null($request->pusher_app_secret) ? '' : $request->pusher_app_secret;
        $pusherData['pusher_app_cluster'] = is_null($request->pusher_app_cluster) ? '' : $request->pusher_app_cluster;
    ...
    public function  appSettings($id=1,Request $request)
    {
        ...
        if(!$appSettings){
            $appSettings = (object) array('api_key' => '', 'api_user'=> '');     
        }

        $appLoginSettings = DB::table('app_login_page')->select(DB::raw("*"))->first();
        if(!$appLoginSettings){
            $appLoginSettings = (object) array('logo' => '', 'title'=> '',' description' => NULL,'fb_login' => 0, 'google_login' => 0,'privacy_policy'=> NULL);     
        }

        return view('admin.app-settings-create',compact('action','id','appSettings','type','appLoginSettings'));
    }

    public function  appSettingUpdate(Request $request)
    {
        $appData = array();
        $appData['api_key'] = is_null($request->api_key) ? '' : $request->api_key;
        $appData['api_user'] = is_null($request->api_user) ? '' : $request->api_user;
